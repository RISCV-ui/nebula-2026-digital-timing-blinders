...
// Decimal print support. Software writes a binary value to 0x10 and
// reads the packed BCD digits back from 0x14 (low 8) and 0x18 (top 2),
// so it can emit a counter without a division routine. Both ends are
// flops, which puts the double-dabble chain on a clk_s4
// register-to-register path.
reg [31:0]uart_bcd_in_reg;
reg [39:0]uart_bcd_reg;
wire [39:0]uart_bcd_digits;

bin_to_bcd_32 uart_decimal(
    .binary (uart_bcd_in_reg),
    .bcd    (uart_bcd_digits)
);
  ...
  if(addr_valid == 1'b1 && read_write == 1'b0)
    begin
     case(addr[7:0])
                 ...
                 8'h10: data_out = uart_bcd_in_reg;
                 8'h14: data_out = uart_bcd_reg[31:0];
                 8'h18: data_out = {24'd0, uart_bcd_reg[39:32]};
...
begin
     if(rst)
     begin
          uart_bcd_in_reg <= 32'd0;
          uart_bcd_reg    <= 40'd0;
     end
     else
     begin
          if(addr_valid && read_write && (addr[7:0] == 8'h10))
               uart_bcd_in_reg <= data_in;
          uart_bcd_reg <= uart_bcd_digits;
     end
end
...
wire _unused_ok = &{1'b0,
                     addr[31:8], uart_divider_reg[31:16],
