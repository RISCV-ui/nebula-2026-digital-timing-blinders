...
/////// peripheral dot -- pipelined internally: dot_out now trails va/vb
/////// by 2 clk_2 cycles (see fp4_dot_unit). The write-enable into the
/////// output fifo is delayed by the same 2 cycles below so the data
/////// (fifo_data_write) and its valid (w_en) stay aligned.
...
wire pop_en = (!fifo_empty_1 && !fifo_empty_2 && !fifo_full);
assign r_en_1 = pop_en;
assign r_en_2 = pop_en;

// Delay line matching the 2-cycle pipeline added inside fp4_dot_unit's
// adder tree: the input fifos are popped this cycle (pop_en), but the
// resulting dot product only becomes valid 2 clk_2 cycles later, so the
// write strobe into fifo_timer_r must wait for it too. Bare, no reset:
// this is state tracking a feed-forward datapath, not architectural state.
reg pop_en_d1, pop_en_d2;
always @(posedge clk_2) begin
    pop_en_d1 <= pop_en;
    pop_en_d2 <= pop_en_d1;
end
assign w_en = pop_en_d2;
