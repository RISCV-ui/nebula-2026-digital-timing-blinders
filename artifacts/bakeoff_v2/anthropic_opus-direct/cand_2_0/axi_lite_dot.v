...
//// issue enable (same expression as before, now the head of a 3 deep
//// shadow pipeline that tracks the 3 new stages inside fp4_dot_unit)
wire        issue_en;
reg         w_en_d1, w_en_d2, w_en_d3;
...
/////// peripheral dot  (now a 3 stage pipeline clocked by clk_2)
...
assign issue_en = (!fifo_empty_1 && !fifo_empty_2 && !fifo_full);
assign r_en_1 = issue_en;
assign r_en_2 = issue_en;

// the dot result now appears 3 clk_2 cycles after the operands are popped,
// so the write strobe into fifo_timer_r is delayed by the same 3 stages.
