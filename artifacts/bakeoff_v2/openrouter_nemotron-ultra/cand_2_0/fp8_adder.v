...
///// combinational logic - restructured for shallower logic depth
always @(*)
begin
    // Default assignments
    shift_a         = 4'd0;
    ...
    // Special case detection - parallel computation
    wire both_zero     = (a[6:0] == 7'd0) && (b[6:0] == 7'd0);
    wire a_zero        = (a[6:0] == 7'd0);
    wire b_zero        = (b[6:0] == 7'd0);
    wire cancel        = (a[7] != b[7]) && (a[6:0] == b[6:0]);
    wire a_inf_nan     = (a[6:0] == 7'b1111111);
    wire b_inf_nan     = (b[6:0] == 7'b1111111);
    wire both_max_exp    = (a[6:3] == 4'b1111) && (b[6:3] == 4'b1111) && (a[2:0] + b[2:0] > 4'd15);
    wire special_case  = both_zero || a_zero || b_zero || cancel || a_inf_nan || b_inf_nan || both_max_exp;
    
    // Special case results
    wire [7:0] special_y;
    assign special_y = both_zero     ? ((a[7] & b[7]) ? 8'b10000000 : 8'b00000000) :
                       a_zero        ? b :
                       b_zero        ? a :
                       cancel        ? 8'd0 :
                       a_inf_nan     ? {a[7], 7'b1111111} :
                       b_inf_nan     ? {b[7], 7'b1111111} :
                       both_max_exp  ? {a[7], 7'b1111111} :
                       8'd0;

    // Main path - compute in parallel with special case detection
    // Exponent alignment
    ...
    // Sticky bits - parallel computation
    sticky_a = (shift_b != 0) && (shift_b >= 8 ? |{mant_a, 4'd0} : |(({mant_a, 4'd0}) & (8'hFF >> (8 - shift_b))));
    sticky_b = (shift_a != 0) && (shift_a >= 8 ? |{mant_b, 4'd0} : |(({mant_b, 4'd0}) & (8'hFF >> (8 - shift_a))));
    ...
    // Leading-one detector: parallel casez
    casez (sum_mant[8:0])
    ...
    // Normalization - flattened decision tree
    if (found) begin
        if ((a[6:3] != 4'd0) || (b[6:3] != 4'd0)) begin
            wire l1_gt_7 = (leading_1_index > 5'd7);
            wire l1_lt_7 = (leading_1_index < 5'd7);
            
            if (l1_gt_7) begin
                norm_shift    = leading_1_index - 5'd7;
                exp_final     = a_grater_reg ? exp_a + norm_shift : exp_b + norm_shift;
                sum_mant_norm = sum_mant >> norm_shift;
                exp_final_2   = exp_final;
            end else if (l1_lt_7) begin
                norm_shift = 5'd7 - leading_1_index;
                exp_final  = a_grater_reg ? exp_a - norm_shift : exp_b - norm_shift;
                ...
                exp_final_2    = a_grater_reg ? {exp_a[5], exp_a} : {exp_b[5], exp_b};
            end
        end else begin
            sum_mant_norm = sum_mant;
            exp_final_2   = (leading_1_index >= 5'd7) ? $signed({2'b00, leading_1_index}) - 7'sd6 : 7'sd0;
        end
    end

    // Rounding - flattened
    mant_rount = (exp_final_2 == 0) ? 
                 {2'b00, sum_mant_norm[7:4]} :
                 {2'b00, sum_mant_norm[7:4]} + {5'b0, (sum_mant_norm[3] & (sum_mant_norm[2] | sum_mant_norm[1] | sum_mant_norm[0] | sum_mant_norm[4]))};

    exp_round = mant_rount[4] ? exp_final_2 + 1 : exp_final_2;

    // Final output mux
    y = special_case ? special_y :
        (exp_round > 15) ? {sing_reg, 7'b1111111} :
        {sing_reg, exp_round[3:0], mant_rount[2:0]};
end
