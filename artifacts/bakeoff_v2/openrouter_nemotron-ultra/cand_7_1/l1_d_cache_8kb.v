...
data_l1_valid     = 1'b0;
     ...
     begin     
          ...
          begin
               ...
               // A write hit completes in this cycle -- mem_en is already
               // asserted for the hit way and read_write_mem_X is high, so the
               // block lands in the data array on this edge. It still has to be
               // ACKNOWLEDGED: data_l1_valid is the only completion signal the
               // level above has, and read_buffer_d_cache's write_back state
               // waits on it before it can move on to the refill. Without this
               // the first dirty eviction hung the whole data path for good.
               // Nothing caught it earlier because the smoke program never
               // evicts a dirty block inside its run.
               data_l1_valid        = 1'b1;
