...
// Pipeline register: breaks the level1->level2 combinational chain.
// Bare register, no reset -- feed-forward datapath, matches the rule
// that newly added pipeline stages carry no reset the original lacked.
...
// Level 2 (now operates on registered level-1 results)
fp8_adder add4 (add_0_r, add_1_r, add_4);
fp8_adder add5 (add_2_r, add_3_r, add_5);

// Pipeline register: breaks the level2->level3 combinational chain.
always @(posedge clk) begin
    add_4_r <= add_4;
    add_5_r <= add_5;
end

// Level 3 (now operates on registered level-2 results)
