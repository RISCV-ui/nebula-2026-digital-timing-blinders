...
\title{\vspace{-12mm}\color{accent}\bfseries
       Constraint Optimisation through RTL Enhancement\\[2pt]
       \large\mdseries Formally gated, LLM-driven timing closure on a
       five-domain SoC benchmark}
\author{Shubhanshu Shivhare \and Nagarjun BV\\[2pt]
        \small Team \textbf{Timing Blinders} \textperiodcentered\ Astera Labs
        Nebula 2026 \textperiodcentered\ Digital Track}
\date{\small Generated \reportgenerated}
...
\numaccepted{} that survived every gate. The result we are most confident in is
not the largest number the flow produced: on the \bestclock{} domain, worst
negative slack improves by \bestdelta\,ns --- $F_{\max}$ from \bestfmaxfrom\,MHz
to \bestfmaxto\,MHz --- and two structurally different rewrites, generated
independently by two different models, agree on that figure to within
\bestagreement\,ns. Agreement at that margin is the reason we report it as an
effect of the edit rather than as placement variance.

The finding we think matters more is negative. \numcontractkills{} rewrites passed
...
Three claims carry the work, and each is stated so that it can be checked
...
\begin{itemize}
  \item \textbf{Five asynchronous masters.} They share no clock tree at
        ...
        on its own. Reporting one global number for five independent closure
        questions is the reporting choice this reveals as wrong.
...
divide ratio \SI{2}{\nano\second} after a rising edge -- the selected clock
high, exactly the moment a combinational mux cuts a pulse -- and toggling the
gate enable at odd phases. The fixed structures produce \runtsfixed{} runt
...
is not a warning in a timing report -- it is a path that silently stops being
analysed.
...
The interesting baseline is not ``no LLM'' --- it is ``the same LLM with the
gates switched off'', because that is what most published RTL-repair loops
are. This report does not have to simulate it: the gates recorded what they
...
survived. Each row is a rewrite that a formal equivalence check passed --- with
a complete proof, not a bounded one --- and that the flow rejected anyway,
...
different rewrites --- different models, different algorithms, hundreds of
diff lines apart --- agree on it to within \bestagreement\,ns.}
...
crosses between the two tables --- mixing measurement generations is the
specific mistake that produced a wrong headline earlier in this project, and
it is not repeated here.

What carries across is the direction. The largest gain in the second
generation is on \vthreeclock{} at \vthreedelta\,ns --- the same domain the
claim above is made on, arrived at through a different synthesis, a different
floorplan and a different placement. \vthreegains{} domains improve and
...
\vthreearea\,\si{\um\squared} of area, which is nothing on a design this
size and is reported rather than rounded away.
...
combinational blocks and the wide AXI interconnect, which is precisely the
difficulty this project exists to attack: the blocks a prover struggles with
...
own macros against the artifacts the demo just read. It is read-only --- no
API key, no network, no toolchain --- and finishes in about a second, because
a demo that depends on a free-tier provider being up is a demo that fails in
...
All four arms ran in one invocation, over the same targets, with an identical
gate configuration, so the rows are comparable --- except where an arm lost
calls to the harness defect, which is disclosed in the table rather than
averaged away.
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item \textbf{A latency change is a legal move, with a price.} The four
        ...
        edits are latency-preserving ones --- which excludes exactly the
        transform that deep combinational paths need.
...
search, written as a \texttt{for} loop, which synthesises to nine serial
compares --- and both replaced it with a single-level parallel priority
match. They agree on the transform and disagree on everything else: one
...
\texttt{a / b} does not shorten the path, it hands the prover a hard
arithmetic theorem instead.
...
Every number in this document is regenerated from the run artifacts by one
command; none is typed into the source. The document fails loudly rather than
silently when an artifact is missing: an unavailable value renders as a
visible \textcolor{critical}{[pending]} marker, not as a zero.
...
# 5. regenerate every table, figure and number in this report
