...
by two different models, agree on that figure to within \bestagreement\,ns.
That margin is below the placement variance measured on this design, so the
change is attributed to the edit.
...
proposal was well-formed and the gates passed, because the tree already
contained the edit and there was nothing left to disprove, so the loop counted
a second closed path. Scope is now derived from the working tree's diff instead
of from the proposal's own module list, and a proposal with an empty diff is
recorded as a no-op (Section~\ref{sec:scope}).
...
that must learn to wait, and the set is proved together. That produced the next failure: rewrites
that were provably equivalent at the module and wrong at the parent. G1b was
added to catch them, and its rejections are Table~\ref{tab:g1b}.

\subsection{The biggest loss in the funnel was ours}

In the first version of Table~\ref{tab:funnel} the largest single cause of
rejection was not a gate but replies that spent their whole token budget inside
reasoning and returned no text, which the harness recorded as a model failure.
The defect hit the reasoning-heavy arms hardest, which biases the comparison
towards the cheaper models. It is fixed in the runs that followed. It is left
in the published funnel with the affected arms marked, rather than removed.

\subsection{Global WNS was not a usable metric on this design}

Global WNS on a five-master design is set by whichever domain is worst, and
here that is the divider path no permitted transform can reach. It also moves
further between two placements of the same netlist than under any accepted
edit. Reporting was changed to per-domain slack, and the headline claim
restricted to a change two independently generated rewrites reproduce. The
second routed generation was run for the same reason: a wider floorplan with
its own baseline, with no figures mixed between generations.
...
\begin{enumerate}[leftmargin=1.4em]
  \item \textbf{Equivalence gate with a recorded proof depth.} An edit enters
        ...
        proved complete at latency${}+2$. Every other case is bounded, and the
        bound is printed next to the result rather than folded into a pass
        count. A flow that accepts on simulation does not separate these three
        cases.

  \item \textbf{A separate gate for the parent's timing contract (G1b).}
        Module-level equivalence is not sufficient, and the gap is measured
        here rather than argued: \numcontractkills{} rewrites passed G1 with a
        complete proof and were rejected by G1b at the parent
        (Table~\ref{tab:g1b}). The three rewrites came from three different
        models, so the defect follows the task rather than one model. This is
  ...
  \item \textbf{Multi-module proposals, so latency-changing edits are
        ...
        parent that must wait for it, and the set is proved together. Without
        that field the only reachable edits are latency-preserving ones, which
        rules out the transform that deep combinational paths need.
